...
class AccountsTypeorm {
    constructor(options) {
        ...
        const setRepositories = () => {
            if (connection) {
                ...
                this.userRepository = typeorm_1.getRepository(userEntity, connectionName);
                this.emailRepository = typeorm_1.getRepository(userEmailEntity, connectionName);
                this.serviceRepository = typeorm_1.getRepository(userServiceEntity, connectionName);
                this.sessionRepository = typeorm_1.getRepository(userSessionEntity, connectionName);
            }
        };
    ...
    async createUser(createUser) {
        const { username, email, password } = createUser, otherFields = tslib_1.__rest(createUser, ["username", "email", "password"]);
    ...
    async setService(userId, serviceName, data, token) {
        ...
        const _a = data, { id = null } = _a, options = tslib_1.__rest(_a, ["id"]);
        if (service) {
    ...
    async setResetPassword(userId, email, newPassword, token) {
